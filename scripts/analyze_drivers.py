...
# ----- features -----
df["log_capacity"]   = np.log(df["home_capacity"].where(df["home_capacity"].notna(), df["capacity"]))
...
f2 = "residual_margin ~ home_b2b + away_b2b + rest_delta_z + log_capacity_z + delta_elev_z + C(home_team) + C(away_team)"
...
f2_elev = "residual_margin ~ home_b2b + away_b2b + rest_delta_z + log_capacity_z + elev_m_z + C(home_team) + C(away_team)"
...
df["delta_bin"] = pd.cut(df["delta_elev"], bins=bins, labels=labels, include_lowest=True)
print("\n[Groupes Δ altitude] mean(residual_margin) par bin:")
print(df.groupby("delta_bin")["residual_margin"].agg(["count","mean","std"]).round(2))

# ----- petite vérif Denver -----
print("\n[Denver focus] mean(residual_margin) par bin adversaire (Δ altitude vus depuis Denver domicile):")
den = df[df["home_team"] == "Denver Nuggets"]
print(den.groupby("delta_bin")["residual_margin"].agg(["count","mean","std"]).round(2))
